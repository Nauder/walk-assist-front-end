...
export default function UpdatePonto({params}: Readonly<{ params: { id: string } }>) {
  ...
  const onSubmit = async () => {
    axios.put(`${localStorage.getItem('servidor')}pontos/${params.id}`, {nome})
      .then(function (response) {
        setErro('');
        setSucesso(`Ponto ${nome} editado com sucesso`)
        refreshPontos();
      })
  ...
  const getInitialData = async () => {
    setIsLoading(true);
    axios.get(`${localStorage.getItem('servidor')}pontos/${params.id}`)
      .then(function (response) {
        console.log(response.data)
        if (response.data.success || response.data.ponto) {
          setNome(response.data.ponto.nome);
          setIsLoading(false);
        } else {
