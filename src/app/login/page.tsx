...
export default function Login() {
    ...
    axios.post(`${localStorage.getItem('servidor')}login`, {registro, "senha": md5(senha)})
      .then(function (response) {
        ...
        if (response.data.success) {
          ...
          setCredentials(response.data.registro);
        } else {
  ...
  const setCredentials = async (registro: string) => {
    axios.get(`${localStorage.getItem('servidor')}usuarios/${registro}`)
      .then(function (response) {
          ...
          <form className="space-y-4 md:space-y-6" action={onSubmit}>
            <div>
              <label htmlFor="endereco"
                     className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">* Endereço
                do
                Servidor</label>
              <input type="text" name="endereco" id="endereco"
                     className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                     placeholder="10.20.8.127" required={true}
